...
NSString *const kXDSMarqueeLabelControllerRestartNotification = @"XDSMarqueeLabelViewControllerRestart";
NSString *const kXDSMarqueeLabelShouldLabelizeNotification = @"XDSMarqueeLabelShouldLabelizeNotification";
NSString *const kXDSMarqueeLabelShouldAnimateNotification = @"XDSMarqueeLabelShouldAnimateNotification";
NSString *const kXDSMarqueeLabelAnimationCompletionBlock = @"XDSMarqueeLabelAnimationCompletionBlock";

// Animation completion block
typedef void(^XDSMLAnimationCompletionBlock)(BOOL finished);

// iOS Version check for iOS 8.0.0
#define SYSTEM_VERSION_EQUAL_TO(v)  ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedSame)

// Helpers
@interface UIView (XDSMarqueeLabelHelpers)
- (UIViewController *)firstAvailableViewController;
- (id)traverseResponderChainForFirstViewController;
@end

@interface CAMediaTimingFunction (XDSMarqueeLabelHelpers)
...
CGPoint XDSMLOffsetCGPoint(CGPoint point, CGFloat offset);

@end


@implementation XDSLabel

#pragma mark - Class Methods and handlers

+ (void)restartLabelsOfController:(UIViewController *)controller {
    [XDSLabel notifyController:controller
                       withMessage:kXDSMarqueeLabelControllerRestartNotification];
}

+ (void)controllerViewWillAppear:(UIViewController *)controller {
    [XDSLabel restartLabelsOfController:controller];
}
...
+ (void)controllerLabelsShouldLabelize:(UIViewController *)controller {
    [XDSLabel notifyController:controller
                       withMessage:kXDSMarqueeLabelShouldLabelizeNotification];
}

+ (void)controllerLabelsShouldAnimate:(UIViewController *)controller {
    [XDSLabel notifyController:controller
                       withMessage:kXDSMarqueeLabelShouldAnimateNotification];
}
...
- (void)setupLabel {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewControllerShouldRestart:) name:kXDSMarqueeLabelControllerRestartNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldLabelize:) name:kXDSMarqueeLabelShouldLabelizeNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(labelsShouldAnimate:) name:kXDSMarqueeLabelShouldAnimateNotification object:nil];
...
- (void)updateSublabelAndLocationsAndBeginScroll:(BOOL)beginScroll {
    ...
    switch (self.marqueeType) {
        case XDSMLContinuous:
        case XDSMLContinuousReverse:
        {
            CGFloat awayLabelOffset;
            if (self.marqueeType == XDSMLContinuous) {
...
- (void)scrollAwayWithInterval:(NSTimeInterval)interval delayAmount:(NSTimeInterval)delayAmount {
    ...
    XDSMLAnimationCompletionBlock completionBlock = ^(BOOL finished) {
        if (!finished) {
    ...
    [awayAnim setValue:completionBlock forKey:kXDSMarqueeLabelAnimationCompletionBlock];
...
- (void)scrollContinuousWithInterval:(NSTimeInterval)interval after:(NSTimeInterval)delayAmount {
    ...
    for (UILabel *sl in labels) {
        ...
        NSArray *values = @[[NSValue valueWithCGPoint:XDSMLOffsetCGPoint(self.homeLabelFrame.origin, offset)],      // Initial location, home
                            [NSValue valueWithCGPoint:XDSMLOffsetCGPoint(self.homeLabelFrame.origin, offset)],      // Initial delay, at home
                            [NSValue valueWithCGPoint:XDSMLOffsetCGPoint(self.awayLabelFrame.origin, offset)]];     // Animation to home
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    XDSMLAnimationCompletionBlock completionBlock = [anim valueForKey:kXDSMarqueeLabelAnimationCompletionBlock];
    if (completionBlock) {
...
- (void)setXDSMarqueeType:(XDSMarqueeType)marqueeType {
    ...
    if (_marqueeType == XDSMLContinuous) {
...
@implementation UIView (XDSMarqueeLabelHelpers)
// Thanks to Phil M
...
@implementation CAMediaTimingFunction (XDSMarqueeLabelHelpers)
